...
For more information on configuring Treasury permissions, see the [Treasury Contracts Documentation](getting-started/treasury-contracts.md).
...
Send the token request as **`application/x-www-form-urlencoded`** (standard for OAuth 2.0 token endpoints):
...
**For Confidential Clients (typically without PKCE; PKCE may still be used in addition to `client_secret` if your client is configured for it):**
...
When the provider returns a `refresh_token`, you can request a new access token:
...
async function refreshAccessToken(refreshToken: string): Promise<TokenInfo> {
  ...
  // Confidential clients: append client_secret to the same form body if your client requires it.
...
* `refresh_token`: Issued refresh token (refresh\_token grant)
...
const STORAGE_KEY_REFRESH = 'xion_oauth_refresh_token'
...
/** Optional: obtain a new access token using refresh_token from a prior exchange. */
export async function refreshAccessToken(): Promise<TokenInfo> {
  const refreshToken = localStorage.getItem(STORAGE_KEY_REFRESH)
  if (!refreshToken) {
    throw new Error('No refresh token stored. Run the authorization flow again.')
  }
  const serverInfo = await getOAuthServerInfo()
  const clientId = getClientId()
  const response = await fetch(serverInfo.token_endpoint, {
...
export function saveTokenInfo(tokenInfo: TokenInfo): void {
  ...
  if (tokenInfo.refreshToken) {
    localStorage.setItem(STORAGE_KEY_REFRESH, tokenInfo.refreshToken)
  }
}
...
export function getTokenInfo(): TokenInfo | null {
  ...
  return {
    ...
    refreshToken: localStorage.getItem(STORAGE_KEY_REFRESH) ?? undefined,
  }
}
...
3. **Token Storage**: Store access tokens securely. Use localStorage for persistence, but be aware of XSS risks. Consider using httpOnly cookies for production applications. If the token response includes a `refresh_token`, persist it separately (as in the example above) so you can rotate access tokens without a full login.
4. **Token Expiration**: Prefer calling `grant_type=refresh_token` when you have a stored refresh token; otherwise re-run the authorization flow when the access token expires.
5. **`TokenInfo` type**: Extend your app's `TokenInfo` type with an optional `refreshToken?: string` if you follow the sample storage helpers.
...
**Response** (shape is illustrative; see [OAuth2 Protected API Documentation](https://oauth2.testnet.burnt.com/protected-api-docs) for the live schema):
...
Use **`id`** as the user's MetaAccount address when you need it for display, CosmJS queries, or building messages (the transaction API overwrites `fromAddress` / `sender` fields from the token for supported message types).
...
`gasUsed` and `gasWanted` are included when available. There is **no** top-level block `height` on this response; use the transaction status endpoint below if you need inclusion height.
...
Queries the status of a previously submitted transaction from the chain (Cosmos SDK tx result).
...
**`code`**: `0` means the transaction executed successfully; non-zero values indicate failure (Cosmos SDK convention). The response may also include a `tx` field with raw transaction data.
...
`fees.amount` is derived from the current gas price configuration; treat numbers as illustrative.
...
// Usage example: Execute a contract method (use GET /api/v1/me → `id` as sender when you need the MetaAccount address)
const message = createExecuteContractMessage(
  'xion1...',           // sender: MetaAccount address from `GET /api/v1/me` (`id`); server may overwrite from the OAuth token
...
// Usage example: Instantiate a CW20 token contract (prefer `GET /api/v1/me` → `id` for sender)
const sender = 'xion1...'
const cw20InitMsg = {
  name: 'My Token',
  symbol: 'MTK',
...
For a list of available RPC endpoints for each network, see the [Public Endpoints and Resources](../references/public-endpoints-and-resources.md) documentation.
...
* [Verona Agent Toolkit](../tools/verona-toolkit.md) — `verona-toolkit` CLI for testnet/mainnet (`--network mainnet`) including OAuth2 client management
